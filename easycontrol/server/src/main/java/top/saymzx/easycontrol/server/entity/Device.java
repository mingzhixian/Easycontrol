...
public final class Device {
  ...
  private static void setClipBoardListener() {
    ClipboardManager.addPrimaryClipChangedListener(new IOnPrimaryClipChangedListener.Stub() {
      public void dispatchPrimaryClipChanged() {
        String newClipboardText = String.valueOf(ClipboardManager.getText());
        if (!newClipboardText.equals(nowClipboardText)) {
          ...
          try {
            ...
            Server.writeFully(Server.audioStream, tmpTextByte, 0, tmpTextByte.length);
          } catch (IOException e) {
            throw new RuntimeException(e);
          }
        }
  ...
  public static void setClipboardText(String text) {
    if (!Objects.equals(nowClipboardText, text)) {
  ...
  public static void touchEvent(int action, Pair<Integer, Integer> position, int pointerId) {
    ...
    pointer.x = (float) position.first * deviceSize.first / videoSize.first;
    pointer.y = (float) position.second * deviceSize.second / videoSize.second;
    ...
    if (pointerCount == 1) {
      ...
      if (action == MotionEvent.ACTION_UP) {
        ...
        newAction =
            MotionEvent.ACTION_POINTER_DOWN | (pointerIndex << MotionEvent.ACTION_POINTER_INDEX_SHIFT);
      }
    }
  ...
  public static boolean setScreenPowerMode(int mode) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
      ...
      if (physicalDisplayIds == null) return false;
      boolean allOk = true;
      for (long physicalDisplayId : physicalDisplayIds) {
        allOk = allOk & SurfaceControl.setDisplayPowerMode(SurfaceControl.getPhysicalDisplayToken(physicalDisplayId), mode);
      }
      return allOk;
    } else {
