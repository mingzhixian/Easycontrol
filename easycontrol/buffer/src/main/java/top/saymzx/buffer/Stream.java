...
public class Stream {
  ...
  public Stream(boolean canWrite, boolean canMultipleSend, UnderlySocketFunction underlySocketFunction) {
    this.canWrite = canWrite;
    this.canMultipleSend = canMultipleSend;
    this.underlySocketFunction = underlySocketFunction;
  }

  public Stream(boolean canWrite, UnderlySocketFunction underlySocketFunction) {
    this.canWrite = canWrite;
    this.canMultipleSend = true;
    this.underlySocketFunction = underlySocketFunction;
  }

  public Stream(UnderlySocketFunction underlySocketFunction) {
    this.canWrite = true;
    this.canMultipleSend = true;
    this.underlySocketFunction = underlySocketFunction;
  }
  ...
  public void pushSource(byte[] bytes) {
    source.write(ByteBuffer.wrap(bytes));
  }
