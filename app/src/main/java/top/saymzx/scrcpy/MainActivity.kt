...
class MainActivity : AppCompatActivity() {
  ...
  private fun sendServer() {
    ...
    // 读取或创建保存配对密钥
    configs.public = File(this.applicationContext.filesDir, "public.key")
    configs.private = File(this.applicationContext.filesDir, "private.key")
    val crypto: AdbCrypto
    if (configs.public.isFile && configs.private.isFile) {
      ...
      crypto.saveAdbKeyPair(configs.private, configs.public)
    }
    // 连接ADB
